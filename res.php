<?php
    ...
    if($mode == "insert"){
        ...
        $query = "INSERT INTO portfolio(name, sitename, subs, part, mission, exp, skill, func, git, sitepage, thum, howlong, responsive
                    ) values(
                        '$name', '$sitename', '$subs', '$part', '$mission', '$exp', '$skill', '$func', '$git', '$sitepage', '$thumstring', '$howlong', '$responsive')";
        mysqli_query($connect, $query);
    }
    if($mode == "delete"){
        $num = $_GET["num"];
    ...
    if($mode == "update"){ 
        ...
        $query = "UPDATE portfolio SET name='$name', sitename='$sitename', subs='$subs', part='$part', mission='$mission', exp='$exp', skill='$skill', func='$func', git='git', sitepage='$sitepage', howlong='$howlong', responsive='$responsive'  WHERE num='$num'";
        mysqli_query($connect, $query);
    }
?>
